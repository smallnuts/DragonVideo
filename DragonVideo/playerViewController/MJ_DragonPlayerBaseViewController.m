...
#import "DragonEverydayEat+CoreDataProperties.h"
...
@property (nonatomic, assign) BOOL isCoreData;
@property (nonatomic, strong) CoreDataManager *coreDataManager;
@property (nonatomic, strong) NSMutableArray *arrCoreData;
...
- (void)viewDidLoad {
    ...
    self.isCoreData = NO;
    self.arrCoreData = [NSMutableArray arrayWithCapacity:0];
    self.lightValue = [UIScreen mainScreen].brightness;
    self.view.backgroundColor = [UIColor whiteColor];
    
    self.coreDataManager = [CoreDataManager shareCoreDataManager];
    [self creatAVPlayer];
    [self.avPlayer addObserver:self forKeyPath:@"loadedTimeRanges" options:NSKeyValueObservingOptionNew context:nil];// 监听loadedTimeRanges属性
...
- (void)creatAnyControlView
{
    ...
    self.headLable.text =[NSString stringWithFormat:@"%@", self.foodDetailModel.course_name];
    ...
    [self.favouriteButton setBackgroundImage:[UIImage imageNamed:@"collect"] forState:UIControlStateNormal];
    //添加事件
...
//先判断该视频是否被收藏，如果被收藏了就会执行下面花括号里的代码
    if (self.isCoreData) {
        [self.favouriteButton setBackgroundImage:[UIImage imageNamed:@"collect"] forState:UIControlStateNormal];
        self.favouriteButton.transform = CGAffineTransformMakeRotation(M_PI_2);
        NSFetchRequest *fetchRequest = [[NSFetchRequest alloc] init];
        NSEntityDescription *entity = [NSEntityDescription entityForName:@"DragonEverydayEat" inManagedObjectContext:self.coreDataManager.managedObjectContext];
        [fetchRequest setEntity:entity];
        NSError *error = nil;
        NSArray *fetchedObjects = [self.coreDataManager.managedObjectContext executeFetchRequest:fetchRequest error:&error];
        if (fetchedObjects == nil) {
            NSLog(@"fetchedObjects == nil  %@", error);
        } else {
            [self.arrCoreData setArray:fetchedObjects];
        }
        for (DragonEverydayEat *everydayEat in self.arrCoreData) {
            if ([everydayEat.course_name isEqualToString:self.foodDetailModel.course_name]) {
                [self.coreDataManager.managedObjectContext deleteObject:everydayEat];
                break;
            }
        }
...
//如果没被收藏就执行下面的代码
        ...
        [self.favouriteButton setBackgroundImage:[UIImage imageNamed:@"collect2"] forState:UIControlStateNormal];
        self.favouriteButton.transform = CGAffineTransformMakeRotation(M_PI_2);
        DragonEverydayEat *everydayEat = [NSEntityDescription insertNewObjectForEntityForName:@"DragonEverydayEat" inManagedObjectContext:self.coreDataManager.managedObjectContext];
        everydayEat.course_image = self.foodDetailModel.course_image;
        everydayEat.course_name = self.foodDetailModel.course_name;
        everydayEat.course_subject = self.foodDetailModel.course_subject;
        everydayEat.course_video = self.foodDetailModel.course_video;
        everydayEat.video_watchcount = self.foodDetailModel.video_watchcount;
        [self.coreDataManager saveContext];
        self.isCoreData = YES;
        self.downloadlable.hidden = NO;
        self.downloadlable.text = @"收藏成功";
        [self.view addSubview:self.downloadlable];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            self.downloadlable.hidden = YES;
        });
...
- (void)downloadButtonAction:(UIButton *)sender {
    ...
    if (self.isDown) {
        ...
        self.isDown = YES;
//上面的代码是判断视频是否下载了，并对相应的按钮状态进行更新。
    NSURL *url = [NSURL URLWithString:self.foodDetailModel.course_video];
//获取到要下载到视频的网络链接地址
...
// 如果要保存文件,需要将文件保存至沙盒
        // 1. 根据URL获取到下载的文件名
        NSString *fileName = [self.foodDetailModel.course_name lastPathComponent];
        // 2. 生成沙盒的路径
    ...
    [self.downLoad resume];
            ...
            self.downloadlable.text = @"当前没有网络";
            [self.view addSubview:self.downloadlable];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                self.downloadlable.hidden = YES;
            });
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    for (int i = 0; i < self.strNameArray.count; i++) {
        if ([self.foodDetailModel.course_name isEqualToString:self.strNameArray[i]]) {
...
- (BOOL)isConnectionAvailable {
    BOOL isisExistenceNetwork = YES;
    
    Reachability *reach = [Reachability reachabilityWithHostName:self.foodDetailModel.course_video];
    switch ([reach currentReachabilityStatus]) {
        case NotReachable:
            isisExistenceNetwork = NO;
            //NSLog(@"notReachable");
            break;
        case ReachableViaWiFi:
            isisExistenceNetwork = YES;
            //NSLog(@"WIFI");
            break;
